  ...
  return (
    <div className={`relative ${props} hidden lg:block`}>
      <div className="flex justify-center items-center h-full space-x-4">
        <button
          ref={profileRef}
          className="w-5 h-5 outline-none rounded-full ring-offset-2 lg:focus:ring-orange-500"
        ...
        >
          <svg
            ...
            aria-hidden="true"
            role="presentation"
            className="w-full h-full rounded-full fill-black-500"
          >
            <path
              fillRule="evenodd"
      ...
      >
        {navigation.map((item, idx) => (
          <li key={idx + 238}>
            <a
              key={idx}
              className="block text-inherit lg:hover:bg-green-500 lg:p-2.5"
              href={item.path}
            >
              {item.title}
            </a>
...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`relative shadow z-30 bg-[#ffffff] w-full lg:fixed lg:text-sm lg:border-none ${
      ...
      >
        <div className="items-center gap-x-5 px-2 max-w-screen-xl mx-auto lg:flex lg:px-4">
          <div className="flex items-center justify-between py-2 lg:py-3 lg:block">
            <a href="#" className="text-xl font-bold text-gray-700 ml-4">
              {"BIT Networking"}
            </a>
            ...
            <ul className="lg:flex lg:space-x-6 lg:space-y-0 justify-between">
              <div className="items-center space-y-6 flex-1 justify-start lg:ml-8 lg:flex lg:space-x-6 lg:space-y-0">
                {navigation.map((item, idx) => {
                  return (
                    <li key={idx}>
                      <button className="w-full lg:flex items-center justify-between gap-1 text-white font-bold hover:text-yellow-500">
                        {item.title}
                      </button>
                    </li>
                  );
              ...
              <div className="flex align-middle items-center justify-center gap-x-6 space-y-3 lg:space-y-0">
                <li>
                  <Link
                    to="/login/alumni"
                    className="block my-3 lg:ml-10 py-[0.4rem] px-3 text-white font-bold text-center bg-[#2F2F35] hover:bg-orange-600 border rounded lg:border-none"
                  >
                    Log In
                  </Link>
                </li>
                <li>
                  <Link
                    to="/signup"
                    className={`block my-3 py-[0.4rem] font-bold px-3 text-center text-white bg-orange-600 hover:bg-orange-700 active:bg-green-800 active:shadow-none rounded shadow lg:inline ${classes.signupbutton}`}
                  >
                    Sign Up
                  </Link>
                </li>
